...
// NOTE: This is WIP generalization of the Apple sample project below:
...
class ClassificationProvider {
    ...
    func loadModel() {
        ...
        let imageSegmentationWrapper = try? Resnet50(configuration: defaultConfig)
        guard let imageSegmentationInstance = imageSegmentationWrapper else {
            fatalError("Failed to create the model instance.")
        }
        let imageSegmentationModel = imageSegmentationInstance.model
        
        // OPEN: consider guard/let
        do {
            self.model = try VNCoreMLModel(for: imageSegmentationModel)
            print("Model loaded successfully")
        } catch {
            print("Error loading model: \(error)")
            fatalError("Failed to create a VNCoreMLModel instance: \(error)")
        }
    }
...
class ViewController: UIViewController {
    private let processor = ClassificationProvider()
